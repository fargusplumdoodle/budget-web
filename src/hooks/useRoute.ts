...
const removeIdTemplateFromPath = (path: string) => path.replace(routeIdSubstitutionString, '');

const removeIdFromPath = (path: string) => path.replace(routeIdValueRegex, '');

const getRouteFromPathname = (pathname: string): RouteType | undefined => Object.values(ROUTES).find((route) => {

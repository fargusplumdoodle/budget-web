...
import {refreshAuthToken, requestAuthToken, resetAuth, setAuth} from "./slice";
...
function* executeRequestAuthToken({payload}: PayloadAction<AuthState>) {
    try {
        const response: AuthState = yield call(authApi.retrieveToken, payload.authCode)
...
function* executeRefreshAuthToken({payload}: PayloadAction<AuthState>) {
    try {
        const response: AuthState = yield call(authApi.refreshToken, payload.refreshToken)
        yield put(setAuth(response))
    } catch {
        yield put(setAuth({...payload, status: 'error'}))
    }
